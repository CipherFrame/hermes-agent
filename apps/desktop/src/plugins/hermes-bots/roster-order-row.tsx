 ...
 * when another roster item is dragged above or below this row.
 */
...
import { $draggingRosterItem, BOT_DRAG_MIME, ROSTER_ITEM_DRAG_MIME } from './roster-order'
...
export function ReorderableRosterRow({ children, itemKey, onReorder }: ReorderableRosterRowProps) {
  const draggingItem = useValue($draggingRosterItem)
  ...
  const accepts = (event: DragEvent) => {
    return (
      ...
      (event.dataTransfer.types.includes(ROSTER_ITEM_DRAG_MIME) || event.dataTransfer.types.includes(BOT_DRAG_MIME))
    )
  }
  ...
  return (
    <div
      ...
      onDrop={event => {
        ...
        const sourceKey =
          event.dataTransfer.getData(ROSTER_ITEM_DRAG_MIME) || event.dataTransfer.getData(BOT_DRAG_MIME) || draggingItem
